...
async def _render_notify_menu(call: CallbackQuery) -> None:
    user_id = await upsert_user(call.from_user.id, call.from_user.username)
    s = await get_settings(user_id)
    assert s is not None
    await call.message.edit_text(
...
@router.callback_query(SettingsCb.filter(F.action == "notif"))
async def cb_notify_menu(call: CallbackQuery, callback_data: SettingsCb) -> None:
