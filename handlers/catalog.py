...
async def _ask_location(
    ...
    # Сообщение-переносчик клавиатуры: текст не «оповещение», а короткий
    # маркер, чтобы снизу появилась reply-клавиатура.
    sent_kb = await msg.answer(
        await t("catalog.nearby.kb_carrier"), reply_markup=loc_kb,
    )
